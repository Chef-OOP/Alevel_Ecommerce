...
namespace ECommerce_Business.Concrete
{
    public class ProductPropertyManager : IProductPropertyService
    {
        ...
        public EntityResult UpDelete(ProductProperty model)
        {
            try
            {
                model.IsDeleted = true;
                model.IsActive = false;
                if (propertyDal.UpDelete(model) > 0)
                    return new EntityResult(ResultType.Success, $"{model.Value} isimli özellik silindi");
