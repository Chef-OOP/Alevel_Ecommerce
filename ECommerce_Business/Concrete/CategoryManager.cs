...
namespace ECommerce_Business.Concrete
{
    public class CategoryManager : ICategoryService
    {
        ...
        public EntityResult UpDelete(Category model)
        {
            try
            {
                model.IsDeleted = true;
                model.IsActive = false;
                if (categoryDal.UpDelete(model) > 0)
                    return new EntityResult(ResultType.Success, $"{model.Name} isimli kategori silindi");
